...
# K Language Web Server Startup Script
# This script handles the complete setup and startup process

set -e

PROJECT_ROOT="$(cd "$(dirname "$0")" && pwd)"
echo "K Language Web Server Setup"
...
if [ ! -d "$PROJECT_ROOT/bin" ] || [ ! -f "$PROJECT_ROOT/bin/web/jettyService/KServlet.class" ]; then
    BUILD_NEEDED=true
elif [ "$PROJECT_ROOT/src" -nt "$PROJECT_ROOT/bin" ]; then
    BUILD_NEEDED=true
fi
...
if [ "$BUILD_NEEDED" = true ]; then
    ...
    # Use Maven to build (compiles Scala first, then Java)
    cd "$PROJECT_ROOT"
    mvn compile -q
...
cd "$PROJECT_ROOT/src/web/jettyService"
