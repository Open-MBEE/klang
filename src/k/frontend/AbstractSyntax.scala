    ...
    if (!exp1.containsSetComprhension && !exp2.containsSetComprhension) {
      ...
      if (UtilSMT.isConstructorPredicate(this)) {
        ...
        op match {
          ...
          case ISIN =>
            // Use seq.contains for Seq types, select for Set/Array types
            val exp2Type = TypeChecker.exp2Type.get(exp2)
            exp2Type match {
              case IdentType(QualifiedName(List("Seq")), _) =>
          ...
          case NOTISIN =>
            ...
            exp2Type match {
              case IdentType(QualifiedName(List("Seq")), _) =>
                s"(not (seq.contains $exp2SMT (seq.unit $exp1SMT)))"
              case _ =>
...
case class CollectionEnumExp(kind: CollectionKind, exps: List[Exp]) extends Exp {
  ...
  override def toJavaString : String = "new " + kind.toJavaString + "(Arrays.asList( " + exps.map(x => x.toJavaString).mkString(",") + "))"
...
case class TypeCastCheckExp(cast: Boolean, exp: Exp, ty: Type) extends Exp {
  ...
  override def toSMT(className: String, subTyping: Boolean): String = {
    if (cast) {
      ...
      val expSMT = exp.toSMT(className, subTyping)
      // Get the source type from the type checker's exp2Type map
      val sourceType = Option(TypeChecker.exp2Type.get(exp)).getOrElse {
        exp match {
          case _: IntegerLiteral => IntType
          case _: RealLiteral => RealType  
          case _: BooleanLiteral => BoolType
          case _ => IntType // Default fallback
        }
      }
      // Generate appropriate SMT conversion
      (sourceType, ty) match {
        // Real -> Int (truncation)
        case (RealType, IntType) => s"(to_int $expSMT)"
        // Int -> Real
        case (IntType, RealType) => s"(to_real $expSMT)"
        // Int -> BitVec
        case (IntType, BitVecType(width)) => s"((_ int2bv $width) $expSMT)"
        // BitVec -> Int
        case (BitVecType(_), IntType) => s"(bv2int $expSMT)"
        // Int -> SignedIntType
 ...
 * Maps to SMT-LIB2 FloatingPoint theory.
 * @param ebits exponent bits (8 for Float32, 11 for Float64)
 * @param sbits significand bits including implicit bit (24 for Float32, 53 for Float64)
 */
case class FloatType(ebits: Int, sbits: Int) extends PrimitiveType {
  require(ebits > 0 && sbits > 1, s"FloatType requires ebits > 0 and sbits > 1, got ($ebits, $sbits)")

  override def statistics() {
    // No specific statistic yet
  }

  // SMT-LIB2 uses (_ FloatingPoint eb sb) syntax
  override def toSMT: String = s"(_ FloatingPoint $ebits $sbits)"

  override def toScala: String = if (ebits == 8 && sbits == 24) "Float" else "Double"

  override def toString = {
    if (ebits == 8 && sbits == 24) "Float32"
    else if (ebits == 11 && sbits == 53) "Float64"
    else s"Float[$ebits,$sbits]"
  }

  override def toJavaString = if (ebits == 8 && sbits == 24) "Float" else "Double"

  override def toJson1 = {
    new JSONObject().put("type", "FloatType").put("ebits", ebits).put("sbits", sbits)
  }
...
object FloatType {
  val Float32 = FloatType(8, 24)   // IEEE 754 single precision
  val Float64 = FloatType(11, 53)  // IEEE 754 double precision
}

/**
 * Fixed-width signed integer type.
 * Semantically a BitVec but with signed arithmetic operations by default.
 * Maps to SMT-LIB2 (_ BitVec N) with signed operations (bvslt, bvsgt, bvsdiv, etc.)
 */
case class SignedIntType(width: Int) extends PrimitiveType {
  require(width > 0, s"SignedIntType width must be positive, got $width")
  ...
  override def toScala: String = width match {
    case 8  => "Byte"
    case 16 => "Short"
    case 32 => "Int"
    case 64 => "Long"
  ...
  override def toString = s"Int$width"

  override def toJavaString = toScala

  override def toJson1 = {
    new JSONObject().put("type", "SignedIntType").put("width", width)
  }

  override def toJson2 = {
    new JSONObject().put("type", "ElementValue").put("element", s"Int$width")
  }
}

/**
 * Fixed-width unsigned integer type.
 * Semantically a BitVec but with unsigned arithmetic operations by default.
 * Maps to SMT-LIB2 (_ BitVec N) with unsigned operations (bvult, bvugt, bvudiv, etc.)
 */
case class UnsignedIntType(width: Int) extends PrimitiveType {
  require(width > 0, s"UnsignedIntType width must be positive, got $width")
  ...
  override def toScala: String = width match {
    case 8  => "Short"  // No unsigned byte in Scala/Java
    case 16 => "Int"
    case 32 => "Long"
    case 64 => "Long"   // BigInt would be needed for full range
    case _  => "Long"
  }

  override def toString = s"UInt$width"

  override def toJavaString = toScala

  override def toJson1 = {
    new JSONObject().put("type", "UnsignedIntType").put("width", width)
  }
