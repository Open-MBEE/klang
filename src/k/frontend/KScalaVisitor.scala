...
class KScalaVisitor extends ModelBaseVisitor[AnyRef] {
  ...
  override def visitThisOuterExp(ctx: ModelParser.ThisOuterExpContext): AnyRef  = {
    var e: Exp = visit(ctx.expression()).asInstanceOf[Exp]
    ThisOuterExp(e)
  }

  override def visitIndexExp(ctx: ModelParser.IndexExpContext): AnyRef = {
    var e = visit(ctx.expression()).asInstanceOf[Exp]
  ...
  override def visitLiteral(ctx: ModelParser.LiteralContext): AnyRef = {
    if (ctx.IntegerLiteral() != null) {
      ...
      DateLiteral(ctx.DateLiteral().getSymbol().getText())
    } else if (ctx.DurationLiteral() != null) {
      DurationLiteral(ctx.DurationLiteral().getSymbol().getText())
    } else {
